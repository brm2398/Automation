...
# Takes split input like "Economist/YouGov" or "Reuters/Ipsos"
# returns poll source
...
# Takes url links from RCP website
html_page = urlopen("https://realclearpolitics.com/epolls/other/2018_generic_congressional_vote-6185.html#polls")
page = soup(html_page)
...
# match the name of the poll to the poll info
# in the 538 file, return the index
...
with open("national_congress_Bri.csv", "r", encoding="utf8") as fp:
...
with open("pollster-ratings_Bri.csv", "r") as fn:
...
# transform highest index from string to int
...
for count in range(0, total_row_num):
	...
	if raceName == "2018 Generic Congressional Vote":
		...
		pollInstance = [pollDate, DemScore, RepScore, pollMethod, pollRating, pollPopulation, pollError, pollSponsor, pollSource, pollLilnks]
...
# add the poll index value to the poll instance
...
with open("national_congress_Bri.csv", "a", newline='') as f:
